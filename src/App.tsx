...
// 最終更新日: 2026-07-21 (Codex) — 生活の全頁を旦那さまの家の保存箱へ渡せるようにする。AIの参照範囲はHouse側で別管理。
...
export default function App() {
  ...
  async function handoffToPartnerHome() {
    ...
    if (!baseUrl) { setNotice("先に旦那さまの家のURLを設定してね。"); return; }
    try {
      ...
      setNotice("灯り帳の" + entries.length + "頁を、旦那さまの家の保存箱へ写したよ。会話のAIにはまだ渡らない。");
    } catch (error) {
...
function Settings({ settings, entries, setSettings, onExport, onImport, onHandoff, onDeleteBefore, onRename }: { settings: AkariSettings; entries: LifeEntry[]; setSettings: (setting: AkariSettings) => void; onExport: () => void; onImport: () => void; onHandoff: () => void; onDeleteBefore: (date: string) => void; onRename: (oldName: string, newName: string) => void }) {
  ...
  return <section className="paper"><p className="eyebrow">設定</p><h1>項目を整える</h1>
    ...
    <section className="backup"><h2>旦那さまの家の保存箱</h2><p>灯り帳の全頁を、パソコンの生活記録棚へ写す。ここで写しただけでは、会話のAIは読まない。AIが何を参照できるかは旦那さまの家側で決める。</p><label className="noteField"><span>旦那さまの家のURL</span><input value={settings.partnerHomeUrl ?? ""} onChange={(event) => setSettings({ ...settings, partnerHomeUrl: event.target.value })} placeholder="接続先のURL" /></label>{settings.partnerHandoffAt && <small>最後に写した日: {new Date(settings.partnerHandoffAt).toLocaleString("ja-JP")}</small>}<div><button className="softButton" onClick={onHandoff}>全頁を保存箱へ写す</button></div><p className="footnote">パソコンで旦那さまの家のAPIが動いている時だけ使える。スマホだけの時は、下のJSON避難を使える。</p></section>
