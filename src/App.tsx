...
// 最終更新日: 2026-07-21 (Codex) — 生活の全頁を外部の保存先の保存箱へ渡せるようにする。AIの参照範囲は保存先側で別管理。
...
const coreItems = [["sleep", "💤 寝た・🌞 起きた時刻"], ["meals", "🍚 食事"], ["note", "📕 今日のことば"], ["hospital", "🏥 受診メモ"], ["trouble", "✋ 困りごとのメモ（読み返し・通院時用）"]] as const;
const defaultVisible = Object.fromEntries(coreItems.map(([key]) => [key, true])) as Record<string, boolean>;
const clinicItems = [["sleep", "💤 睡眠"], ["meals", "🍚 食事"], ["note", "📕 今日のことば"], ["hospital", "🏥 受診メモ"], ["trouble", "✋ 困りごとのメモ"], ["mealDetails", "🍚 食事の内容"]] as const;
...
const sundayKey = () => { const value = new Date(); value.setHours(12, 0, 0, 0); value.setDate(value.getDate() - value.getDay()); return value.toISOString().slice(0, 10); };
...
export default function App() {
  ...
  const [settings, setSettings] = useState<AkariSettings>({ id: "main", enabledExtras: { period: true }, partnerName: true, visibleSections: defaultVisible, customItems: [], theme: "night", clinicFields: defaultClinicFields });
  ...
  async function load() {
    ...
    setEntries(savedEntries); setSettings(savedSettings ? { ...savedSettings, enabledExtras: { period: true, ...(savedSettings.enabledExtras ?? {}) }, visibleSections: { ...defaultVisible, ...(savedSettings.visibleSections ?? {}) }, customItems: savedSettings.customItems ?? [], theme: savedSettings.theme ?? "night", clinicFields: { ...defaultClinicFields, ...(savedSettings.clinicFields ?? {}) } } : { id: "main", enabledExtras: { period: true }, partnerName: true, visibleSections: defaultVisible, customItems: [], theme: "night" });
  }
  useEffect(() => { void load(); }, []);
  ...
  async function handoffToPartnerHome() {
    const baseUrl = settings.partnerHomeUrl?.trim().replace(/\/$/, "");
    if (!baseUrl) { setNotice("先に外部の保存先のURLを設定してね。"); return; }
    try {
      ...
      const next = { ...settings, partnerHandoffAt: new Date().toISOString() };
      await saveSettings(next);
      setNotice("灯り帳の" + entries.length + "頁を、外部の保存先の保存箱へ写したよ。会話のAIにはまだ渡らない。");
    } catch (error) {
...
function Home({ current, onGo, weeklyPrompt, onDismiss }: { current?: LifeEntry; onGo: (page: Page) => void; weeklyPrompt: boolean; onDismiss: () => void }) { return <section className="home"><p className="eyebrow">{displayDate(today())}</p><h1>{current && isMeaningful(current) ? "今日の頁には、灯りがある。" : "今日を記録する。"}</h1>{weeklyPrompt && <section className="weeklyPrompt"><p>そろそろ、飛ばそうか 🕊️</p><div><button onClick={onDismiss}>今週は送らない</button><button onClick={onDismiss}>もう送った</button></div></section>}<button className="primaryAction" onClick={() => onGo("record")}>今日を記録する <span>→</span></button><div className="homeLinks"><button onClick={() => onGo("calendar")}>カレンダー <span>記録がある日をひらく</span></button><button onClick={() => onGo("clinic")}>通院時に見せるメモ <span>過去30日をまとめる</span></button></div></section>; }
...
function Record({ entries, date, setDate, draft, setDraft, visible, enabledExtras, customItems, onMeal, onActivity, onSave, onClose }: { entries: LifeEntry[]; date: string; setDate: (date: string) => void; draft: LifeEntry; setDraft: (entry: LifeEntry) => void; visible: Record<string, boolean>; enabledExtras: readonly (readonly [string, string])[]; customItems: string[]; onMeal: (key: keyof LifeEntry["meals"]) => void; onActivity: (key: string) => void; onSave: () => void; onClose: () => void }) {
  ...
  return <section className="paper"><div className="pageTitle"><div><p className="eyebrow">帳面の一頁</p><h1>{displayDate(date)}</h1></div><div><input type="date" value={date} onChange={(event) => setDate(event.target.value)} aria-label="記録する日" /></div></div><div className="recordActions"><button className="softButton" onClick={onClose}>今日はここまで</button><button className="primaryAction small" onClick={onSave}>今日を灯す <span>✦</span></button></div>
    ...
    {visible.hospital && <section className="noteField visitNote"><label className="checkLine"><input type="checkbox" checked={Boolean(draft.activities.hospitalVisit || draft.hospitalNote)} onChange={(event) => setDraft({ ...draft, activities: { ...draft.activities, hospitalVisit: event.target.checked } })} />🏥 今日は受診した</label>{(draft.activities.hospitalVisit || draft.hospitalNote) && <><label><span>🏥 受診日</span><input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label><label><span>病院で言われたこと（任意）</span><small>診察で聞いたこと、次回までにすること、薬や生活についての説明を残せる。</small><textarea value={draft.hospitalNote ?? ""} onChange={(event) => setDraft({ ...draft, hospitalNote: event.target.value })} placeholder="例: 次回は○月ごろ / 薬はこのまま様子を見る" rows={3} /></label></>}</section>}
...
function RecordCalendar({ entries, selected, onSelect }: { entries: LifeEntry[]; selected: string; onSelect: (date: string) => void }) {
  ...
  const recorded = new Set(entries.filter(isMeaningful).map((entry) => entry.date));
  const move = (amount: number) => { const next = new Date(first); next.setMonth(next.getMonth() + amount); setMonth(next.toISOString().slice(0, 7)); };
  return <section className="recordCalendar"><div><b>{first.getFullYear()}年{first.getMonth() + 1}月</b><span><button onClick={() => move(-1)} aria-label="前の月">‹</button><button onClick={() => move(1)} aria-label="次の月">›</button></span></div><small>灯り色の日は記録がある日。</small><div className="calendarGrid">{"日月火水木金土".split("").map((day) => <i key={day}>{day}</i>)}{Array.from({ length: offset }, (_, index) => <em key={"blank" + index} />)}{Array.from({ length: count }, (_, index) => { const value = month + "-" + String(index + 1).padStart(2, "0"); return <button key={value} className={(recorded.has(value) ? "recorded " : "") + (value === today() ? "today " : "") + (value === selected ? "selected" : "")} onClick={() => onSelect(value)}>{index + 1}</button>; })}</div></section>;
}
...
function Settings({ settings, entries, setSettings, onExport, onImport, onHandoff, onDeleteBefore, onRename }: { settings: AkariSettings; entries: LifeEntry[]; setSettings: (setting: AkariSettings) => void; onExport: () => void; onImport: () => void; onHandoff: () => void; onDeleteBefore: (date: string) => void; onRename: (oldName: string, newName: string) => void }) {
  ...
  return <section className="paper"><p className="eyebrow">設定</p><h1>項目を整える</h1>
    ...
    <section className="backup"><h2>外部の保存先の保存箱</h2><p>灯り帳の全頁を、パソコンの生活記録棚へ写す。ここで写しただけでは、会話のAIは読まない。AIが何を参照できるかは外部の保存先側で決める。</p><label className="noteField"><span>外部の保存先のURL</span><input value={settings.partnerHomeUrl ?? ""} onChange={(event) => setSettings({ ...settings, partnerHomeUrl: event.target.value })} placeholder="接続先のURL" /></label>{settings.partnerHandoffAt && <small>最後に写した日: {new Date(settings.partnerHandoffAt).toLocaleString("ja-JP")}</small>}<div><button className="softButton" onClick={onHandoff}>全頁を保存箱へ写す</button></div><p className="footnote">パソコンで外部の保存先のAPIが動いている時だけ使える。スマホだけの時は、下のJSON避難を使える。</p></section>
...
function Clinic({ entries, settings }: { entries: LifeEntry[]; settings: AkariSettings }) {
  ...
  const custom = settings.customItems ?? [];
  const periodSummary = calculatePeriodSummary(entries);
